...
class TestReservation(Reservation):
    def __init__(self, renewed, vrp, zone, start, remaining_second):
        super(TestReservation, self).__init__()

        self._renewed = renewed
        self._vrp = vrp
        self._zone = zone
        self._start = start
        self._remaining_second = remaining_second
        self._id = id
    ...
    @property
    def end(self):
        return self.start + (self.remaining_second * 1000)
    ...
    @property
    def remaining_second(self):
        return self._remaining_second

    @remaining_second.setter
    def remaining_second(self, value):
        self._remaining_second = value
...
class TestParkingClient(ParkingClient):
    def __init__(self):
        super(TestParkingClient, self).__init__()
    ...
    def _update_reservations(self, tick):
        if self._state_num == 1:
            self._state_num = 2
            self._reservations = [
                TestReservation(renewed=False,
                                vrp='Е445ТА159',
                                zone=101,
                                start=1,
                                remaining_second=60 * 60),
                TestReservation(renewed=False,
                                vrp='Т226КТ59',
                                zone=101,
                                start=1,
                                remaining_second=90 * 60)
            ]

            return

        for reservation in self._reservations:
            reservation.remaining_second -= 60

            if reservation.remaining_second <= 0:
                continue

        for reservation in list([r for r in self._reservations if r.remaining_minutes <= 0]):
    ...
    def get_reservations(self):
        ...
        self._update_reservations(tick=self._measure_number)
